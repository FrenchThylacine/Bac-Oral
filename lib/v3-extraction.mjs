...
function detectProcedes(text) {
  ...
  const found = [];
  const usedAnalyses = new Set();

  // First, try priority procédés
  for (const proc of PRIORITY_PROCEDES) {
    if (found.length >= 10) break;
    const idx = lower.indexOf(proc.toLowerCase());
    if (idx !== -1) {
      const analysis = getAnalysis(proc);
      if (!usedAnalyses.has(analysis)) {
  ...
  // Then secondary
  for (const proc of SECONDARY_PROCEDES) {
    if (found.length >= 10) break;
    const idx = lower.indexOf(proc.toLowerCase());
    if (idx !== -1) {
      const analysis = getAnalysis(proc);
  ...
  // Fill with genre-specific if needed
  const genreBank = GENRE_PROC_BANK[detectGenre(text)] || [];
  for (const proc of genreBank) {
    if (found.length >= 10) break;
    const analysis = getAnalysis(proc);
    if (!usedAnalyses.has(analysis)) {
      found.push({
...
function ensureMovements(movements, text) {
  const list = (movements || []).slice(0, 3);
  if (list.length >= 3) return list;
  const partLength = Math.max(1, Math.floor((text || "").length / 3));
  while (list.length < 3) {
    const start = list.length * partLength;
    const end = (list.length + 1) * partLength;
    const block = (text || "").slice(start, end);
    list.push(buildMovement(block, list.length + 1));
  }
