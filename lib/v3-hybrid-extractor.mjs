...
function parseALText(text, sourceFile, method = 'unknown') {
  ...
  const introduction = extractSection(lines, 'INTRODUCTION') || generateIntro(title);
  const conclusion = extractSection(lines, 'CONCLUSION') || generateConclusion(title);
  
  let movements = parseMovements(lines);
  // Fix field name: convert 'name' to 'title'
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentMovement && line.match(/^\s*[-•]\s+Procedure\s*:/i)) {
      ...
      if (procMatch) {
        ...
        // Map color field to colorDetected
        procedure.colorDetected = procedure.color || 'none';
        delete procedure.color;
        delete procedure.name; // Remove duplicate field
