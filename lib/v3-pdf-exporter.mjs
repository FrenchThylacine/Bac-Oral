...
function drawSectionBar(doc, text, color, y) {
  ...
  doc.fillColor("#FFFFFF").font("Helvetica-Bold").fontSize(9)
     .text(text, 46, y + 5, { width: doc.page.width - 92 });
...
function renderAL(doc, al, isFirst) {
  ...
  for (const mov of movements) {
    ...
    for (const proc of procs.slice(0, 10)) {
      const weight = proc.weight || 3;
      const dots = "●".repeat(weight) + "○".repeat(5 - weight);
      const line = `• ${proc.label}${proc.quote ? ` [${proc.quote}]` : ""}  →  ${proc.analysis || "—"}  ${dots}`;
      const needed = doc.heightOfString(line, { width: W - 12 }) + 3;

      if (y + needed > doc.page.height - 60) { doc.addPage(); y = 40; }

      // Color by weight
      const textColor = weight >= 4 ? "#14532D" : weight >= 3 ? "#1E3A8A" : "#374151";
