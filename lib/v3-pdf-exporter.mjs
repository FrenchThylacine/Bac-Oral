...
// ── Convert weight (1-5) to visual dots ────────────────────────
function weightToDots(weight = 3) {
  const w = Math.min(5, Math.max(1, weight || 3));
...
function renderAL(doc, al, isFirst) {
  ...
  for (const mov of movements) {
    ...
    for (const proc of procs.slice(0, 10)) {
      ...
      const dots = weightToDots(weight);
      const line = `• ${proc.label} → ${proc.analysis || "—"}    ${dots}`;
